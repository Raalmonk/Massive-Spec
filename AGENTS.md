...
* The timeline axis starts *before* the pull. FF Logs starts an FFXIV fight at the first pre-pull action, not at the engage; `combatTime` is the fight length excluding pre-pull, so `duration - combatTime` is the pre-pull window (`Fight.pre_pull`). Every timestamp is zeroed on `Fight.zero_time_rel` (combat start), which puts pre-pull casts on negative timestamps. In the frontend every time->x conversion is `(t - timelineStart) * zoom`; never use a bare `t * zoom`.
