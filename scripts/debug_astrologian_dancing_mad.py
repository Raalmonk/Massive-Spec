#!/usr/bin/env python
"""Debug Dancing Mad Astrologian rankings and write the frontend JSON."""
...
async def debug_astrologian_dancing_mad() -> None:
    ...
    print("Dancing Mad Astrologian debug complete")
    ...
    if not ranking.fights:
        print("No fights loaded. This is expected until Dancing Mad has public kill rankings.")
    elif not any(phase_counts):
        print("No native FF Logs phases found for these fights.")


async def main() -> None:
    try:
        await debug_astrologian_dancing_mad()
    finally:
