#!/usr/bin/env python
"""Force refresh Dancing Mad Reaper rankings and write the frontend JSON."""
...
async def refresh_reaper_dancing_mad(limit: int, metric: str) -> Path:
    ...
    if not ranking.fights:
        raise RuntimeError("No Dancing Mad Reaper fights loaded. FF Logs returned no public rankings.")
    ...
    print("Dancing Mad Reaper refresh complete")
    ...
    print(f"casts per fight: {cast_counts[:10]}")
    print(f"phases per fight: {phase_counts[:10]}")
...
async def main() -> None:
    ...
    try:
        await refresh_reaper_dancing_mad(limit=args.limit, metric=args.metric)
    finally:
